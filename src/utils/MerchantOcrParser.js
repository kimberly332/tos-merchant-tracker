...
const extractItemQuantity = (line) => {
  ...
  // If no direct match, look for number next to the item
  const adjacentNumberMatch = line.match(/(\d+)\s*$/);
  return adjacentNumberMatch ? parseInt(adjacentNumberMatch[1]) : 1;
};
